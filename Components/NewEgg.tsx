...
function NewEgg() {
  const [selectedEggs, setSelectedEggs] = useState<EggInfo[]>(() => {
    const initialEggs: EggInfo[] = eggColors.map((color) => ({
      color,
      count: 0,
    }));
    return initialEggs;
  ...
  function handleCountChange(
    ...
    setSelectedEggs((prevSelectedEggs) => {
      const updatedSelectedEggs = prevSelectedEggs.map((egg) =>
        egg.color === color ? { ...egg, count } : egg
      );
      console.log(updatedSelectedEggs)
        ...
        <div>
          <form className="space-y-4" onSubmit={postEggInfo}>
            ...
            {eggColors.map((color, index) => {
              const selectedEgg = selectedEggs.find((egg) => egg.color === color);
              const count = selectedEgg ? selectedEgg.count : 0;
